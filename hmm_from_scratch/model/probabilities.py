...
class ProbabilityVector:
    def __init__(self, probabilities: dict) -> None:
        ...
        assert abs(sum(probs) - 1.0) < 1e-12, "Probabilities must sum to 1."
        assert len(list(filter(lambda x: 0 <= x <= 1, probs))) == len(
            probs), "Probabilities must be numbers from [0, 1] interval."
...
class ProbabilityMatrix:
    def __init__(self, prob_vec_dict: dict):
        assert len(prob_vec_dict) > 1, "The number of input probability vectors must be greater than one."
        assert len(set([str(x.states) for x in
                        prob_vec_dict.values()])) == 1, "All internal states of all vectors must be identical."
    ...
    def __getitem__(self, observable: str) -> np.ndarray:
        if observable not in self.observables:
            raise ValueError("Requesting unknown probability observable from matrix.")
